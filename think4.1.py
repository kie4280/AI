...
class Network:
    ...
    def train(self, data, example):
        ...
        if os.path.isfile("data.pkl") and self.load:
            ...
            self.synapses.append(np.random.randn(self.node_num, example.shape[-1]))
        ...
        for iterator in range(10001):
            ...
            self.synapses.reverse()
            self.layers.reverse()
        self.output = self.layers[-1] * Y_max
        plot.plot(num, cost)
        plot.show()
...
network = Network(3, 3, load=False)
network.train(X, Y)
network.save()
